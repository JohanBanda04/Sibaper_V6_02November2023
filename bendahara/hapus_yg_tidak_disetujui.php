...
if(isset($_POST['hapus_yg_tidak_disetujui'])){
    ...
    if(count($result_data_yg_akan_dihapus)>0){
        ...
        $get_data_sementara = mysqli_query($koneksi,"select * from sementara 
where id_sementara=$_POST[hapus_yg_tidak_disetujui]");
        $result_data_sementara = mysqli_fetch_assoc($get_data_sementara);

        $jmlh_permintaan_on_sementara_not_approved = $result_data_sementara['jumlah'];
