...
sementara.status_acc in ('Penyerahan Barang Ke Pengguna')) ");
    ...
    $data_serah_barang_ke_pengguna_dan_selesai = mysqli_query($koneksi,"select * from `sementara` 
where unit='$unit' and user_id='$user_id_pemohon' and tgl_permintaan='$tgl' and status_acc in
('Penyerahan Barang Ke Pengguna','Selesai') and bendahara_id='$_SESSION[user_id]'");

    $data_setuju_kasub_dan_selesai = mysqli_query($koneksi,"select * from `sementara` 
where unit='$unit' and user_id='$user_id_pemohon' and tgl_permintaan='$tgl' and status_acc in
('Setuju Kasub Bendahara','Selesai') and bendahara_id='$_SESSION[user_id]'");

    $data_status_in_bendahara = mysqli_query($koneksi,"select * from `sementara` 
where unit='$unit' and user_id='$user_id_pemohon' and tgl_permintaan='$tgl' and status_acc in
('Penyerahan Barang Ke Pengguna','Selesai','Setuju Kasub Bendahara','Penerimaan Barang Dari Bendahara') and bendahara_id='$_SESSION[user_id]'");

    //echo mysqli_num_rows($data_status_in_bendahara)."<br>";

    $array_selesai_only = array();
    $array_except_selesai_only = array();
   while($dt = mysqli_fetch_assoc($data_status_in_bendahara)){
       //echo $dt['status_acc']."<br>";
       if($dt['status_acc']=="Selesai"){


           array_push($array_selesai_only,"status selesai");
       } else if($dt['status_acc']!="Selesai"){


           array_push($array_selesai_only,"bukan status selesai");
       }
   }
   //die;
   // $jumlah

//    echo "selesai only: ". count($array_selesai_only)."<br>";
//    echo "bukan selesai only: ". count($array_except_selesai_only)."<br>";
//
//    if(in_array("bukan status selesai",$array_selesai_only)){
//        echo "masih ada yg blm slsai";
//    }

    /*nteh*/
//    echo mysqli_num_rows($data_serah_barang_ke_pengguna_dan_selesai)."<br>";
//    echo mysqli_num_rows($data_setuju_kasub_dan_selesai)."<br>";
//
//    echo (mysqli_num_rows($data_serah_barang_ke_pengguna_dan_selesai)+mysqli_num_rows($data_setuju_kasub_dan_selesai))."<br>";
//
//    echo $jml_request."<br>";
//    echo count($array_pengajuan_kasub_bendahara)."<br>";
//    echo count($array_serah_brg_ke_pengguna)."<br>";
...
//        $mail->Send();
    } else if(count($array_serah_brg_ke_pengguna)==$jml_request || (mysqli_num_rows($data_serah_barang_ke_pengguna_dan_selesai) + mysqli_num_rows($data_setuju_kasub_dan_selesai))==$jml_request){
...
where id_subbidang='$pemohon_data[subbidang_id]'");
            ...
            $mail->Username = "senderforemail340@gmail.com"; //user email
            $mail->Password = "mtklbsimtazfowfi"; //password email
            $mail->SetFrom("senderforemail340@gmail.com", "Pemberian Barang Ke "
                .$pemohon_data['jabatan_detail']." ".$result_subbidang_pemohon['nama_subbidang']); //set email pengirim
            $mail->Subject = "Pemberian Barang Ke ". $pemohon_data['jabatan_detail']." "
                .$result_subbidang_pemohon['nama_subbidang']; //subyek email
            $mail->AddAddress($email_pemohon);  // email tujuan

            $mail->MsgHTML("Halo, ".$pemohon_data['jabatan_detail']." "
                .$result_subbidang_pemohon['nama_subbidang']." Akan Dilakukan Pemberian Barang oleh : "
        ...
        while($data_email_pemohon = mysqli_fetch_array($query_get_email_penggunas)){
            ...
            $mail->MsgHTML("Halo, ".$dt_nama_lengkap
                ." Terdapat Konfirmasi Penerimaan Barang Selesai dari Pengelola : "
                .$_SESSION['nama_lengkap'].", pada tanggal "
                .tanggal_indo($tgl_sekarang).",  segera cek aplikasi SIBAPER melalui akun Anda"); //pesan
            $mail->Send();
        }
    } else {
            ...
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="text-center">
                        ...
                        ?>
                        Konfirmasi Permintaan <b><?php echo $nama_lengkap_pemohon; ?></b></h3><br>
                    <center>
